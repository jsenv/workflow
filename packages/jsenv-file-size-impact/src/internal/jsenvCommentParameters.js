...
const formatSizeImpactAsPercentage = ({ sizeBeforeMerge, sizeAfterMerge }) => {
  ...
  const sizeDiffAsPercentageFormatted = `${
    sizeDiffAsPercentage < 0 ? `-` : "+"
  }${formatPercentage(Math.abs(sizeDiffAsPercentage))}%`
  return sizeDiffAsPercentageFormatted
}

const formatPercentage = (number) => {
  return roundNumber(number, determineDecimals(number))
}
